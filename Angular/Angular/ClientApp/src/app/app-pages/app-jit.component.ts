...
export class JitComponent implements OnInit, OnChanges {
  ...
  public static run() {                        // запуск компиляции
      while ( JitComponent.queue.length > 0 ) {
        JitComponent.queue.shift()();
      }
  }
  /**
  ...
  ngOnChanges( changes: SimpleChanges ) {
    ...
    const link = function() {
        ...
        const module = NgModule({
            imports:        [ JitComponent.build, AppUiModule   ].concat([
